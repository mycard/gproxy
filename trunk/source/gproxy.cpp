...
CGProxy :: ~CGProxy( )
{
	...
	for( vector<CIncomingGameHost *> :: iterator i = m_Games.begin( ); i != m_Games.end( ); i++ )
		delete *i;
	for( vector<CTwoWayConnection *> ::iterator i = m_BNFTPs.begin( ); i != m_BNFTPs.end( ); i++ )
		delete *i;
...
bool CGProxy :: Update( long usecBlock )
{
	...
	if ( m_WC3 )
	{
	    if(!m_BNET->HasError( ) && m_BNET->GetConnected( ))
	    {
		    m_WC3->SetFD( &fd , &send_fd, &nfds );
		    NumFDs++;
		}
	}

	// 7. the custom bnet socket
	...
	if( NumFDs == 0 )
		MILLISLEEP( 50 );
#pragma region GProxyGameConnection
...
#pragma endregion GProxyGameConnection
#pragma region Warcraft III Connection
	CTCPSocket *CNewSocket = m_WC3Server->Accept( &fd );
	if (CNewSocket)
	{
	/*	if (m_WC3)
		{
			m_BNFTPs.push_back( new CTwoWayConnection( CNewSocket, m_Server ,6112));
		}
		else
		{
			m_WC3 = CNewSocket;
			m_WC3->SetNoDelay(true);
			CONSOLE_Print("[GPROXY] Accepted warcraft 3 connection from [" + m_WC3->GetIPString( ) + "]");
			m_BNET = new CTCPClient( );
			m_BNET->SetNoDelay(true);
			m_BNET->Connect(string( ),m_Server,6112);
		}*/
		m_BNFTPs.push_back( new CTwoWayConnection( CNewSocket, m_Server ,6112));
	}
	if (m_WC3)
	{
		...
		if( m_WC3->HasError( ) || !m_WC3->GetConnected( ) )
		{
			CONSOLE_Print( "[GPROXY] local WC3 client disconnected" );
			delete m_WC3;
			m_WC3 = NULL;
			CONSOLE_Print("[GPROXY] Disconnecting the bnet socket");
			m_BNET->Disconnect();
			m_BNET->Reset( );
			m_BNET->SetNoDelay(true);
		}
		else
		{
			...
			if (m_BNET->HasError( ))
			{
				CONSOLE_Print( "[GPROXY] Disconnected from bnet server due to socket error" );
				m_WC3->Disconnect();
				CONSOLE_Print("[GPROXY] Disconnecting the wc3 socket");
				delete m_WC3;
				m_WC3 = NULL;
				m_BNET->Reset( );
				m_BNET->SetNoDelay(true);
			...
			if (m_BNET->GetConnecting( ))
			{
			//	CONSOLE_Print("[GPROXY] Connecting to ["+m_BNET->GetIPString( )+ "]");
				if(m_BNET->CheckConnect( ))
				{
					CONSOLE_Print("[GPROXY] Connected to bnet server ["+ m_BNET->GetIPString( ) +"] :D");
				}
			}
			if (m_BNET->GetConnected( ))
			{
				m_BNET->DoRecv(&fd);
				ExtractBNETPackets( );
				m_BNET->DoSend( &send_fd );
			}
			
			m_WC3->DoSend( &send_fd );
		}
	}
	for(vector<CTwoWayConnection * > ::iterator i = m_BNFTPs.begin( ) ; i != m_BNFTPs.end( );)
	{
...
#pragma endregion Warcraft III Connection
	return m_Exiting;
}
...
void CGProxy :: ProcessWC3Packet(CCommandPacket *packet )
{
	bool forward = true;
	forward = true;
	switch (packet->GetID( ))
	...
	if (forward)
	{
//		CONSOLE_Print("[BNET] Received " +UTIL_ToString(packet->GetData( ).size( )) + " Data: "+ UTIL_ByteArrayToHexString(packet->GetData( )));
		if (ValidateLength(packet->GetData( )))
		m_BNET->PutBytes(packet->GetData( ));
		else
			CONSOLE_Print("[WC3] Wrong length");
	}
}
void CGProxy :: ProcessBNETPacket(CCommandPacket *packet )
{
	bool forward = true;
	switch (packet->GetID( ))
	{
...
//		CONSOLE_Print("[BNET] Received " +UTIL_ToString(packet->GetData( ).size( )) + " Data: "+ UTIL_ByteArrayToHexString(packet->GetData( )));
		if (ValidateLength(packet->GetData( )))
		m_WC3->PutBytes(packet->GetData( ));
		else
			CONSOLE_Print("[BNET] Wrong length");
	}
}
void CGProxy :: ExtractBNETPackets()
{
	string *RecvBuffer = m_BNET->GetBytes( );
	BYTEARRAY Bytes = UTIL_CreateByteArray( (unsigned char *)RecvBuffer->c_str( ), RecvBuffer->size( ));
...
void CGProxy :: ExtractWC3Packets()
{
	string *RecvBuffer = m_WC3->GetBytes( );
	BYTEARRAY Bytes = UTIL_CreateByteArray( (unsigned char *)RecvBuffer->c_str( ), RecvBuffer->size( ));
...
void CGProxy :: SendChatCommand( string message )
{
	if (m_BNET->GetConnected( ))
	{
		BYTEARRAY packet;
...
void CGProxy :: Handle_SID_GETADVLISTEX(BYTEARRAY data)
{
...
void CGProxy :: Handle_SID_CHATEVENT(BYTEARRAY data)
{
...
void CGProxy :: Handle_SID_CHATCOMMAND(BYTEARRAY data)
{
	if (ValidateLength(data))
	{
		...
		{
			m_BNET->PutBytes(data);
		}
	}
}
bool CGProxy :: ProcessCommand(string Message)
{
	string Command;
	string Payload;
	...
	{
		SendLocalChat("GProxy++ Version " + m_Version );
		forward = false;
	}
...
bool CGProxy :: AssignLength( BYTEARRAY &content )
{
...
bool CGProxy :: ValidateLength( BYTEARRAY &content )
{
...
///
/// CBNFTConnection
CBNFTPConnection ::CBNFTPConnection(CTCPSocket *nLocal,string Server ) 
{
	m_Local = nLocal;
	m_Local->SetNoDelay(true);
	m_Remote = new CTCPClient( );
	m_Remote->SetNoDelay(true);
	m_Remote->Connect( string( ), Server, 6112 );
	CONSOLE_Print("[BNFTP] New bnftp connection with the bnet server started");
	m_Delete = false;
}
CBNFTPConnection :: ~CBNFTPConnection ( )
{
	delete m_Local;
	delete m_Remote;
}
unsigned int CBNFTPConnection :: SetFD( void *fd, void *send_fd, int *nfds )
{
	unsigned int NumFDs = 0;
	if( !m_Local->HasError( ) && m_Local->GetConnected( ) )
	{
		m_Local->SetFD( (fd_set *)fd, (fd_set *)send_fd, nfds );
		NumFDs++;
	}
	if (!m_Remote->HasError( ) && m_Remote->GetConnected( ) )
	{
		m_Remote->SetFD( (fd_set *)fd, (fd_set *)send_fd, nfds );
		NumFDs++;
	}
	return NumFDs;
}
void CBNFTPConnection :: Update( void *fd, void *send_fd )
{
	/*if ( m_Local->HasError( ) || !m_Local->GetConnected( ) )
	{
		CONSOLE_Print( "[BNFTP] Local socket disconnected" );
		m_Remote->Disconnect( );
		m_Delete = true;
		return;
	}
	...
	{
		m_Local->DoRecv( (fd_set *)fd );
		m_RemoteBuffer.push( m_Local->GetBytes( ) );
		while ( !m_LocalBuffer.empty( ) )
		{
		  m_Local->PutBytes( *m_LocalBuffer.front( ) );
		  m_LocalBuffer.pop( );
		}
		if ( m_Remote->HasError( ) )
		{
			CONSOLE_Print( "[BNFTP] Remote socket disconnected due to socket error" );
			m_Local->Disconnect( );
			m_Delete = true;
			return;
		}
		if( !m_Remote->GetConnecting( ) && !m_Remote->GetConnected( ) )
		{
			CONSOLE_Print( "[BNFTP] Remote socket disconnected" );
			m_Local->Disconnect( );
			m_Delete = true;
			return;
		}
		if( m_Remote->GetConnected( ))
		{
			m_Remote->DoRecv( (fd_set *)fd );
			m_LocalBuffer.push( m_Remote->GetBytes( ) );
			while ( !m_RemoteBuffer.empty( ) )
			{
				m_Remote->PutBytes( *m_RemoteBuffer.front( ) );
				m_RemoteBuffer.pop( );
			}
			m_Remote->DoSend( (fd_set *)send_fd);

		}
		if ( m_Remote->GetConnecting( ) )
		{
			if ( m_Remote->CheckConnect( ) )
			{
				CONSOLE_Print( "[BNFTP] Remote socket connected" );
			}
		}
		m_Local->DoSend( (fd_set *)send_fd );
	}
	*/
	UpdateLocal( (fd_set * )fd, (fd_set * )send_fd );
	UpdateRemote( (fd_set * )fd, (fd_set * )send_fd);
	if (m_Local->GetConnected( ) )
	{
		m_Local->DoSend( (fd_set * )send_fd );
	}
	if (m_Remote->GetConnected( ) )
...
void CBNFTPConnection :: UpdateLocal( void *fd , void *send_fd )
{
	if ( !m_Local->HasError( ) )
	{
		CONSOLE_Print( "[BNFTP] Local socket disconnected due to socket error" );
		m_Remote->Disconnect( );
		m_Delete = true;
		return;
	}
...
void CBNFTPConnection :: UpdateRemote( void *fd, void *send_fd )
{
	if ( m_Remote->GetConnecting( ) )
	{
		if ( m_Remote->CheckConnect( ) )
...
CTwoWayConnection ::CTwoWayConnection(CTCPSocket *socket, std::string hostname, uint16_t port)
{
	m_FirstSocket = socket;
	m_FirstSocket->SetNoDelay( true );
	m_SecondSocket = new CTCPClient( );
	m_SecondSocket->SetNoDelay( true );
	m_SecondSocket->Connect( string( ) , hostname, port );
	CONSOLE_Print("[GPROXY] Initiating the two way connection" );
}
CTwoWayConnection ::~CTwoWayConnection( )
{
	delete m_FirstSocket;
	delete m_SecondSocket;
}
unsigned int CTwoWayConnection ::SetFD(void *fd, void *send_fd, int *nfds) 
{
	unsigned int NumFDs = 0;
	if ( !m_FirstSocket->HasError( ) && m_FirstSocket->GetConnected( ) )
	{
		m_FirstSocket->SetFD( (fd_set * )fd, (fd_set *)send_fd, nfds );
		NumFDs++;
	}
    if ( !m_SecondSocket->HasError( ) && m_SecondSocket->GetConnected( ) )
	{
		m_SecondSocket->SetFD( (fd_set * )fd, (fd_set *)send_fd, nfds );
		NumFDs++;
	}
	return NumFDs;
}

bool CTwoWayConnection ::Update(void *fd, void *send_fd) 
{
	if ( m_FirstSocket->HasError( ) )
	{
		CONSOLE_Print("[GPROXY] Socket ["+ m_FirstSocket->GetIPString( ) +"] disconnected due to socket error" );
		m_SecondSocket->Disconnect( );
		return true; // exiting
	}
	if ( !m_FirstSocket->GetConnected( ) )
	{
		CONSOLE_Print("[GPROXY] Socket ["+ m_FirstSocket->GetIPString( ) +"] disconnected" );
		m_SecondSocket->Disconnect( );
		return true; // exiting
	}
	if ( m_FirstSocket->GetConnected( ) )
	{
		m_FirstSocket->DoRecv( (fd_set * )fd );
		string *RecvBuffer = m_FirstSocket->GetBytes( );
	    BYTEARRAY Bytes = UTIL_CreateByteArray( (unsigned char *)RecvBuffer->c_str( ), RecvBuffer->size( ) );
		m_TempBuffer.push( Bytes );
	}
	if ( m_SecondSocket->HasError( ) )
	{
		CONSOLE_Print("[GPROXY] Socket ["+ m_SecondSocket->GetIPString( ) +"] disconnected due to socket error" );
		m_FirstSocket->Disconnect( );
		return true;
	}
	if ( m_SecondSocket->GetConnecting( ) )
	{
		if (m_SecondSocket->CheckConnect( ) )
		{
			CONSOLE_Print("[GPROXY] Socket ["+ m_SecondSocket->GetIPString( ) +"] connected" );
		}
	}
	if ( !m_SecondSocket->GetConnected( ) && !m_SecondSocket->GetConnecting( ) )
	{
		CONSOLE_Print("[GPROXY] Socket ["+ m_SecondSocket->GetIPString( ) +"] disconnected" );
		m_FirstSocket->Disconnect( );
		return true;
	}
	if ( m_SecondSocket->GetConnected( ) )
	{
		m_SecondSocket->DoRecv( (fd_set *)fd );
		string *RecvBuffer = m_SecondSocket->GetBytes( );
	    BYTEARRAY Bytes = UTIL_CreateByteArray( (unsigned char *)RecvBuffer->c_str( ), RecvBuffer->size( ) );
		m_FirstSocket->PutBytes( Bytes );
		while ( !m_TempBuffer.empty( ) )
		{
			m_SecondSocket->PutBytes( m_TempBuffer.front( ) );
			m_TempBuffer.pop( );
		}
	}
	m_SecondSocket->DoSend( (fd_set * )send_fd );
	m_FirstSocket->DoSend( (fd_set * )send_fd );
	return false;
}